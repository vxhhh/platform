...
<script>
    export default {
        ...
        methods:{
            page(currentPage){
                const _this = this
                axios.get('http://localhost:8181/product/list/'+ currentPage +'/'+this.pageSize).then(function (resp) {
        ...
        created() {
            const _this = this
            axios.get('http://localhost:8181/product/list/1/'+this.pageSize).then(function (resp) {
                _this.tableData = resp.data.data
                _this.total = resp.data.total
            })
        }
